...
CREATE TABLE IF NOT EXISTS `mydb`.`Patient` (
  ...
  INDEX `Address_id_idx` (`Addressid` ASC) VISIBLE,
  INDEX `Contact_id_idx` (`Contactid` ASC) VISIBLE,
  CONSTRAINT `Personid`
...
CREATE TABLE IF NOT EXISTS `mydb`.`Inpatient` (
  ...
  INDEX `Lab_No_idx` (`Patient_id` ASC, `Lab_No` ASC) VISIBLE,
  CONSTRAINT `Patient_id`
    FOREIGN KEY (`Patient_id`)
    REFERENCES `mydb`.`Patient` (`idPatient`)
    ON DELETE NO ACTION
  ...
  CONSTRAINT `Room_No`
    ...
    ON UPDATE NO ACTION,
  CONSTRAINT `Lab_No`
    FOREIGN KEY (`Patient_id` , `Lab_No`)
    REFERENCES `mydb`.`Lab` (`Patient_id` , `Lab_No`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
